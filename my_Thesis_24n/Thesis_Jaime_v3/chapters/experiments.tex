...
\textcolor{red}{The analytical approach has shown to be more sensitive to the tuning due to its higher non-normalized velocities. However, a set of parameters that lead to good behavior in the tested scenarios has been found. If testing in other scenarios \rewrite{reprase this} shows that this set of parameters does not work or it is too sentitive an option to consider is a normalized velocity with higher nmax. For that somehow it has to be guaranteed that the robot will not go over a maximum in its trajectory to the goal position. This does not affect the computational efficiency since the computationally expensive approach is the numerical one cz it needs to compute 4 masses in each iteration. \rewrite{I have to mention before that the computational problem is the numerical approach only}}
