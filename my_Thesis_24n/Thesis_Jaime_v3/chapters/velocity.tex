...
%However, in the tested cases in this thesis, the descent direction using the Moore-Penrose pseudoinverse caused a less oscillatory behavior. 
However, the mass weighted projector tends to minimize poorly w.r.t. the linear axis, like it happened at torque level in Section \ref{sec:Gradientbasedminimization}.\rewrite{this has to be in corcondance with the the torque level}  Between RG and PG with the Moore-Penrose pseudoinverse there are not significant differences in the rate of convergence, since they reach minima not far from each other.  \\ 
