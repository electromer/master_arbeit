...
Implementing both methods in a MATLAB script shows that the PG requires approximately \textcolor{red}{XX} s, while \textcolor{red}{XX} needs only XX s. \TODO{hacer mediciones random en matlab y meterlas} The speed of convergence is hard to measure because the number of iterations to reach a minimum is different in both methods. This happens always as soon as the redundancy degree is more than one \cite{reduced_gradient}. 
...
\TODO{si me da tiempo hacer un apartado extra en el capitulo de experimentos para comparar PG and RG(analytical) para velocidades normalizadas. Similar a los experimentos de  \cite{reduced_gradient}}
